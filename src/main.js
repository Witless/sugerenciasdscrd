...
client.on('message', async msg => {
  ...
  if (!channel)
    return msg.channel.send('No se ha encontrado el canal para sugerencias.');
  ...
  case 'sugerir':
    if (!args[0])
      return msg.channel.send('Debes introducir una sugerecnia.');
  ...
  case 'aceptar':
    ...
    const approveMsg =
        await channel.messages.fetch(await db.get(args[0]).message);
    if (!approveMsg)
      return msg.channel.send('No se ha encontrado la sugerencia.');
    embed.setAuthor(msg.author.tag, msg.author.avatarURL())
        .setTitle('Sugerencia Aceptada')
        .setColor('GREEN')
        .setDescription(await db.get(args[0]).content);
    if (args[1])
      embed.addField('Razón', args.slice(1).join(' '))

      const approveDone =
          await approveMsg.edit(embed).then(() => true).catch(() => false);
    if (!approveDone)
      return msg.channel.send(
          'No he podido editar el mensaje, por favor, comprueba mis permisos.');
    const approveUnreact = await approveMsg.reactions.removeAll()
                               .then(() => true)
                               .catch(() => false);
    if (!approveUnreact)
      return msg.channel.send(
  ...
  case 'posible':
    ...
    if (args[1])
      ...
      const possibleDone =
          await possibleMsg.edit(embed).then(() => true).catch(() => false);
    if (!possibleDone)
      return msg.channel.send(
          'No he podido editar el mensaje, por favor, comprueba mis permisos.');
    const possibleUnreact = await possibleMsg.reactions.removeAll()
                                .then(() => true)
                                .catch(() => false);
    if (!possibleUnreact)
      return msg.channel.send(
  ...
  case 'denegar':
    ...
    if (args[1])
      ...
      const denyDone =
          await denyMsg.edit(embed).then(() => true).catch(() => false);
    if (!denyDone)
      return msg.channel.send(
          'No he podido editar el mensaje, por favor, comprueba mis permisos.');
    const denyUnreact =
        await denyMsg.reactions.removeAll().then(() => true).catch(() => false);
    if (!denyUnreact)
      return msg.channel.send(
          'No he podido quitar las reacciones del mensaje, por favor, comprueba mis permisos, necesito poder administrar mensajes.');
    break;
