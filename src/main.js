...
client.on("message", async (msg) => {
  ...
  switch (command) {
    ...
    case "aceptar":
      ...
      const approveMsg = await channel.messages.fetch(
        await db.get(args[0]).message
      );
      if (!approveMsg)
      ...
      const approveDone = await approveMsg
        .edit(embed)
        .then(() => true)
        .catch(() => false);
      if (!approveDone)
      ...
      const approveUnreact = await approveMsg.reactions
        .removeAll()
        .then(() => true)
        .catch(() => false);
      if (!approveUnreact)
    ...
    case "posible":
      ...
      const possibleMsg = await channel.messages.fetch(
        await db.get(args[0]).message
      );
      if (!possibleMsg)
        return msg.channel.send("No se ha encontrado la sugerencia.");
      ...
      const possibleDone = await possibleMsg
        .edit(embed)
        .then(() => true)
        .catch(() => false);
      if (!possibleDone)
      ...
      const possibleUnreact = await possibleMsg.reactions
        .removeAll()
        .then(() => true)
        .catch(() => false);
      if (!possibleUnreact)
    ...
    case "denegar":
      ...
      const denyMsg = await channel.messages.fetch(
        await db.get(args[0]).message
      );
      if (!denyMsg)
        return msg.channel.send("No se ha encontrado la sugerencia.");
      ...
      const denyUnreact = await denyMsg.reactions
        .removeAll()
        .then(() => true)
        .catch(() => false);
      if (!denyUnreact)
