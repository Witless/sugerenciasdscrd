...
module.exports = {
  ...
  run : async (client, message, args, channel, pre_channel) => {
    ...
    embed.setAuthor(embed_author_name, embed_author_icon)
        ...
        .setFooter(`${denymessage.reactions.cache.get('👍').count} 👍 | ${
            denymessage.reactions.cache.get('👎').count}  👎`)
    if (args[1]) embed.addField("Razón:", args.slice(1).join(" "))

    message.react("✅")

    const denyDone =
        denymessage.edit(embed).then(() => true).catch(() => false);
    ...
    const denyUnreact =
        denymessage.reactions.removeAll().then(() => true).catch(() => false);
