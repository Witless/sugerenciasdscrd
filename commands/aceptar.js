...
module.exports = {
  ...
  run: async (client, message, args, channel, pre_channel) => {
    if (!message.member.roles.cache.has(config.rol)) return;
    ...
    embed
      ...
      .setFooter(
        `${approvemessage.reactions.cache.get("👍").count} 👍 | ${
          approvemessage.reactions.cache.get("👎").count
        }  👎`
      );
    ...
    const approveDone = approvemessage
      .edit(embed)
      .then(() => true)
      .catch(() => false);
    ...
    const approveUnreact = approvemessage.reactions
      .removeAll()
      .then(() => true)
      .catch(() => false);
